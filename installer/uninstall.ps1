...
if ($userPath -like "*$binDir*") {
    ...
    Write-Host "[OK] Removed from PATH and DATARA_HOME" -ForegroundColor Green
}
...
if (Test-Path $dtrKey) {
    ...
    Write-Host "[OK] Removed .dtr registry association" -ForegroundColor Green
}

$progKey = "HKCU:\Software\Classes\DataraSourceFile"
if (Test-Path $progKey) {
    Remove-Item -Path $progKey -Recurse -Force
    Write-Host "[OK] Removed DataraSourceFile registry class" -ForegroundColor Green
}
