...
| Параметр / Метрика | Rust (rustc 1.98.0) | C++ (MSVC 19.50.35727) | Datara (forgen v1.3.0) | Datara LLVM AOT (`--llvm`) |
|---|---|---|---|---|
| **Флаги компиляции** | `-O -C target-cpu=native` | `/O2 /Oi /Ot /GL /arch:AVX2` | `forgen run` (Cranelift JIT) | `forgen build --llvm -O3` |
| **Разрешение рендера** | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) |
| **Количество попаданий** | **104 040** | **104 040** | **104 040** | **104 040** |
| **Контрольная сумма** | **15 602 392.502885** | **15 602 392.502885** | **15 602 392.502885** | **15 602 392.502885** |
| **Математическое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** |
| **Время выполнения (мс)** | **6 мс** | **6 мс** | **100 мс (JIT в ОЗУ)** | **6 мс (AOT скорость железа)** |
| **Пропускная способность** | **80 000 000 лучей/с** | **80 000 000 лучей/с** | **4 800 000 лучей/с** | **80 000 000 лучей/с** |
| **Аллокации в куче** | 0 байт (zero alloc) | 0 байт (zero alloc) | 0 байт (zero alloc) | 0 байт (zero alloc) |
