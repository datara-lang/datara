...
- **Многострочные объявления**: одно объявление C-функции, у которого возвращаемый тип, список параметров и завершающая точка с запятой разнесены по нескольким строкам исходного заголовка, разбирается корректно (включая многострочные поля структур и typedef-ы):
  ...
  ```
- **Защита звукости (v1.3.1)**: C-функции, возвращающие по значению структуру больше одного 64-битного машинного слова, отклоняются на этапе компиляции с диагностикой `E0962` в любой форме объявления, поскольку нативный бэкенд пока не реализует скрытый sret-слот возвращаемого значения.
