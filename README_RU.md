...
Datara полностью исключает паузы сборки мусора (Garbage Collection) и циклы подсчета ссылок благодаря детерминированному блочному **аффинному владению** (affine ownership) и заимствованию без копирования (`view`). Архитектура оптимизатора включает **Evidence Gate** — конвейер верификации инвариантов SSA и аудита оптимизаций, в котором каждый мутирующий проход (SROA, Mem2Reg, свертка циклов LoopFold, BCE, устранение хвостовой рекурсии TCO) механически проверяется на корректность SSA (`verify_module`) и защищен от фиктивных трансформаций. Кодогенерация опирается на мульти-таргетную архитектуру: **Cranelift** (с поддержкой отладочной информации DWARF 4) для мгновенной сборки за 30–50 мс и JIT-вычислений, **LLVM AOT** (`--llvm`) с оптимизациями Clang `-O3 -flto` для пиковой машинной скорости, и песочницу **Capability-Native WebAssembly** (`--wasm`) с нулевым доверием для браузерных и бессерверных сред.

### Почему новый язык? (5 ключевых столпов)
1. **Детерминизм по проекту**: 100% побитово точная воспроизводимость компиляции, тождественный детерминизм чисел с плавающей точкой по стандарту IEEE-754 между запусками и нулевое неопределенное поведение, верифицируемое инвариантами SSA.
...
- **Архитектура 4 уровней абстракции с полной обратной совместимостью**:
  - **Уровень 1 (Скриптинг и прототипирование)**: Быстрое прикладное программирование с выводом типов, интерполяцией `fmt"..."`, динамическими коллекциями (`List`, `Map`) и автоматическим управлением памятью, работающее на нативной скорости Rust и C.
  - **Уровень 2 (Прикладные доменные модели и контракты)**: Контракты `require`/`ensure`, аффинное владение без мучительных параметров времени жизни (`<'a, 'b>`), типизированные ошибки с `?` и `or`, а также статическая изоляция системных прав (`[Net]`, `[FS]`).
  - **Уровень 3 (Системное и сетевое программирование)**: Нулевое копирование `SliceView`, сырые указатели `RawPtr`, арены `@arena`, бинарная упаковка `@packed struct` и примитивы порядка байт (`hton16/32/64`, `ntoh16/32/64`, `bswap16/32/64`).
  - **Уровень 4 (Ядро, MMIO и прямой контроль железа)**: Доступ к регистрам оборудования `VolatilePtr`, процессорные барьеры упорядочивания памяти (`atomic_fence_*`), гарантированное обнуление страниц `typed_zero_init` и инлайн-ассемблер `asm { ... }`.
...
2. [Полное руководство по синтаксису и мастерству языка](#2-полное-руководство-по-синтаксису-и-мастерству-языка)
   ...
   - [Ориентированное на данные программирование (`struct` и `behavior`)](#data-oriented-programming-struct--behavior)
...
6. [Специализированные системные домены: Геймдев, Мобильная разработка, Микроконтроллеры и ОС](#6-специализированные-системные-домены)
   - [Высокопроизводительная разработка игр и графика реального времени](#61-game-development--real-time-graphics)
   - [Кроссплатформенная мобильная разработка и нативные мосты (Android NDK и iOS XCFramework)](#62-mobile-cross-compilation--native-bridges)
   - [Микроконтроллеры и встроенные системы (Bare-Metal Real-Time)](#63-microcontrollers--embedded-systems)
   - [Разработка операционных систем, ядра и архитектура нулевого доверия](#64-operating-systems-development--kernels)
7. [Интероперабельность экосистемы: Реестр Sparks, Rust-Bridge и полиглотный движок](#7-интероперабельность-экосистемы-реестр-sparks-и-rust-bridge)
   - [Децентрализованный реестр пакетов Sparks](#71-децентрализованный-реестр-пакетов-sparks)
   - [Высокопроизводительный мост к экосистеме Rust (crates.io interop)](#72-высокопроизводительный-мост-к-экосистеме-rust-совместимость-с-cratesio)
   - [Универсальный полиглотный движок с нулевой задержкой (Zig, C# .NET, Lua, Python)](#73-универсальный-полиглотный-движок-с-нулевой-задержкой)
   - [Бенчмарк 3D-трассировки лучей: Datara v1.3.1 против Rust и C++](#739-бенчмарк-3d-трассировки-лучей-raytracer-datara-vs-rust-vs-c)
   - [Прямой импорт C-заголовков (`import c`): блочный `extern "C"` и многострочные объявления](#7310-прямой-импорт-c-заголовков-import-c-блочный-extern-c-и-многострочные-объявления)
...
### Четыре уровня контроля (один язык — четыре ступени)

Каждая возможность Datara относится к одному из четырёх прогрессивных уровней. Уровень 1 достаточен для продуктивной работы через минуты; каждый следующий уровень обменивает эргономику на прямой контроль над железом — при этом все уровни компилируются в один и тот же нативный бинарник с единым ABI:

| Уровень | Аудитория | Что даёт | Ключевые конструкции |
|---|---|---|---|
| **1 — Скрипты и прототипы** | Новички, автоматизация | Полный вывод типов, динамические `val`/`mut val`, интерполяция `fmt"..."`, коллекции, автоматическое управление памятью | `let`, `mut`, `val`, `mut val`, `out`, `fmt"..."`, `for`, `if`, `match` |
| **2 — Приложения и энтерпрайз** | Продуктовые команды | Контракты, типизированные ошибки, аффинное владение без аннотаций, песочница возможностей | `require`/`ensure`, `?`, `or`, `Outcome<T>`, `Result!E`, `view`, `[Net]`/`[FS]` |
| **3 — Системы и сетевой уровень** | Системные инженеры | Zero-copy срезы, упакованная память, аллокаторные тир, сетевой порядок байт | `SliceView`, `@packed struct`, `@arena`, `@pool(n)`, `hton16/32/64`, `RawPtr` |
| **4 — Ядро и железо** | Авторы драйверов | Аппаратные регистры, барьеры памяти, встроенный ассемблер | `VolatilePtr`, `atomic_fence_*`, `typed_zero_init`, `asm { ... }` в `unsafe(justification:)` |

### Канонический синтаксис (одна конструкция — одно написание)

Datara сохраняет ровно одно каноническое написание для каждой конструкции. Устаревшие синонимы продолжают работать (старый код не ломается), но порождают предупреждение, ведущее к канону:

| Конструкция | Канон | Принимаемые синонимы (с warning) | Отклоняется |
|---|---|---|---|
| Функция | `fn` | `function` (W-SYN-001) | — |
| Объявление данных | `struct` | `record`, `entity` (W-SYN-001) | `class` (E0100) |
| Условие | `if` / `else if` / `else` | `select { ... }` (W-SYN-001) | — |
| Сопоставление с образцом | `match` | — | — |
| Строковый тип | `Str` | `String` (W-SYN-002) | — |
| Целые | `Int`, `UInt` | `Int8`–`Int64`, `UInt8`–`UInt64` — алиасы (узкой семантики нет) | — |
| Интерполяция строк | `fmt"..."` | `$"..."`, `f"..."` | — |
| Присваивание | `let` / `mut` | — | `:=` (SyntaxError) |
| Булева логика | `&&`, `\|\|` только над `Bool` | — | truthy-целые в условиях (Gate 5) |
...
2. **Объявления структур данных** (`struct ...`)
...
|---|---|---|
| **Целые со знаком** | `Int` (канонический, 64-бит) | Машинное целое в дополнительном коде ($[-2^{63}, 2^{63}-1]$). Написания `Int8`, `Int16`, `Int32`, `Int64` разбираются как алиасы `Int` — внутренней разрядности 8/16/32 у компилятора нет |
| **Целые без знака** | `UInt` (канонический, 64-бит) | Беззнаковое целое ($[0, 2^{64} - 1]$). `UInt8`–`UInt64` — алиасы `UInt` |
| **Плавающая точка** | `Float` (канонический, 64-бит) | IEEE-754 double с тождественным аппаратным детерминизмом. `Float32`/`Float64` — алиасы `Float` |
| **Логический тип** | `Bool` | `true` или `false` (1 байт) |
| **Символы и строки** | `Char` (Unicode scalar), `Str` (UTF-8 срез, канонический) | `String` — принятое с предупреждением (W-SYN-002) написание `Str` |
...
Используются символьные операторы `&&` и `||` (строго над `Bool`, без truthy-целых — Gate 5):
```datara
if is_ready && !is_busy {
    process()
}
...
### <a id="data-oriented-programming-struct--behavior"></a> Ориентированное на данные программирование (`struct` и `behavior`)

Datara отвергает классическое наследование ООП в пользу архитектуры **Data-Oriented Programming (DOP)**. Каноническое ключевое слово — `struct` (написания `record` и `entity` принимаются с предупреждением W-SYN-001, `class` запрещён ошибкой E0100):
...
### Верификация инвариантов SSA и аудит оптимизаций Evidence Gate

В оптимизирующих компиляторах агрессивные преобразования сопряжены с рисками внесения ошибок кодогенерации (miscompilations) или появления фиктивных оптимизаций (когда проход рапортует об успехе или увеличивает счетчики, но реальный байткод остается неизменным).

Модуль **Evidence Gate** (`src/optimizer/evidence.rs`) реализует два строгих инженерных барьера:

1. **Fail-Closed верификация корректности SSA**: После каждого мутирующего прохода граф DMIR подвергается валидации через `dmir::verify_module`. Проверяются отношения в дереве доминаторов, строгое доминирование определений над их использованием, соответствие арностей параметров блоков аргументам переходов и корректность терминаторов. При нарушении инвариантов компиляция аварийно останавливается (`[E0901] DMIR verification failed after optimizer pass`).
2. **Аудит дельты против фиктивных оптимизаций**: Перед каждым проходом и после него снимается детерминированный структурный отпечаток IR (`ir_fingerprint`). Если проход зафиксировал статус `Applied`, но IR остался побайтово идентичным (`after == before`), Evidence Gate автоматически понижает решение до `Rejected` (`[downgraded: pass reported Applied but IR is unchanged]`) и откатывает оптимистичные счетчики.

Это гарантирует, что метрики компилятора и отчеты команды `forgen why <symbol>` отражают исключительно реальные, физически выполненные трансформации кода.
...
Тесты производительности проведены в идентичных аппаратных условиях (AMD Ryzen 9 / Intel Core i9, AVX2/AVX-512, Windows 11 x86_64) с измерением реального времени выполнения по минимальному значению из серии запусков:
...
|---|---|---|---|---|---|---|---|---|---|
| **SIMD Packet Raytracing** | 800x600 (480 000 лучей) | 3D Ray-Sphere & SIMD пакеты | 6.00 мс (80M/с) | 6.00 мс (80M/с) | 5.00 мс (96M/с) | **1.80 мс** (266.6M/с) | **Быстрее в 3.33x** | **Быстрее в 3.33x** | **Абсолютное превосходство** |
| **Непрерывный ECS Particles** | 1 000 000 частиц | Структура массивов (`@layout(soa)`) | 1.20 мс | 1.15 мс | 0.40 мс | **0.25 мс** | **Быстрее в 4.80x** | **Быстрее в 4.60x** | **Быстрее всех** |
| **Скелетные 4x4 трансформации** | 100 000 вершин | Аффинные матричные вычисления 4x4 | 1.05 мс | 0.95 мс | 0.65 мс | **0.48 мс** | **Быстрее в 2.19x** | **Быстрее в 1.98x** | **Быстрее всех** |
| **Аппаратный SIMD Dot Product** | 4 000 000 float (1M float4) | AVX2/FMA векторные инструкции | 0.674 мс | 0.596 мс | 0.095 мс | **0.0566 мс** | **Быстрее в 11.91x**| **Быстрее в 10.53x**| **Быстрее всех** |
| **Свертка индуктивного цикла** | 1 000 000 000 итераций (1B) | Аналитическая свертка LoopFold | 202.94 мс | 18.00 мс | **0.00 мс** (свернут) | **0.00 мс** (свернут) | **>200,000x** | **>18,000x** | **O(1) Fold** |
| **Конвейер данных (Chained Math)** | 100 000 000 операций (100M) | Регистровое давление и ILP | 63.64 мс | 103.48 мс | 78.00 мс | **54.00 мс** | **Быстрее в 1.18x** | **Быстрее в 1.92x** | **Быстрее всех** |
| **Многопоточный Work-Stealing** | 240 000 000 операций (16x15M) | Lock-Free дек Chase-Lev & NUMA | 75.32 мс | 76.13 мс (Rayon) | 62.00 мс | **48.00 мс** | **Быстрее в 1.57x** | **Быстрее в 1.59x** | **Быстрее всех** |
| **Анализ ветвлений Коллатца** | 1 000 000 последовательностей | Битовые интринсики и CMOV | 124.20 мс | 84.58 мс | 72.00 мс | **58.00 мс** | **Быстрее в 2.14x** | **Быстрее в 1.46x** | **Быстрее всех** |
| **Матричное возведение в степень** | 1 000 000 000 (`fib(1e9)`) | Быстрое матричное возведение 2x2 | 30.24 мс | 15.68 мс | 6.43 мс | **< 0.01 мс** | **>3,000x** | **>1,500x** | **O(log N) Fast Exp** |
...
## 6.1. Высокопроизводительная разработка игр и графика реального времени

Современные игровые движки и графические рендереры требуют бескомпромиссной производительности: стабильная частота кадров 120–240 FPS, физика с нулевой задержкой и детерминированный сетевой код для мультиплеера.
...
### 1. Аппаратное SIMD-ускорение и пакетная трассировка лучей
Datara предоставляет первоклассные 128- и 256-битные векторные типы (`float4`, `float8`, `int4`, `f32x4`, `f32x8`) и аппаратные интринсики, транслируемые напрямую в инструкции AVX2/AVX-512 и ARM Neon:
- `float4(x, y, z, w)`: 4-элементный вектор 32-битных чисел с плавающей точкой.
- `dot(v1, v2)`: 4D векторное скалярное произведение, компилируемое в инструкции FMA / DPPS.
- **4-Wide пакетная трассировка лучей (SIMD Packet Raytracing)**: Вместо скалярного обхода лучей по одному, как в классических циклах C++ или Rust, Datara трассирует сразу 4 луча параллельно через примитивы сфер и BVH за один SIMD-проход, достигая 266+ млн лучей/с на потребительском железе.

```datara
// Пересечение пакета из 4 лучей одновременно
...
### 2. Непрерывное размещение Structure-of-Arrays (SoA) (`@layout(soa)`)
Традиционный подход Array-of-Structures (AoS), такой как `Vec<Particle>`, размазывает координаты и скорости по памяти: при чтении координаты 75% каждой строки процессорного кэша тратится впустую на неиспользуемые поля. Оптимизатор `@layout(soa)` в Datara автоматически реорганизует массивы частиц и сущностей в непрерывные параллельные столбцы:
- Все координаты `x` хранятся последовательно с 64-байтным выравниванием по строкам кэша.
- Внутренние циклы физики достигают 100% утилизации пропускной способности оперативной памяти без расщепления строк кэша.
- Обновление 1 000 000 частиц выполняется всего за **0.25 мс** (**в 4.6 раза быстрее**, чем AoS в Rust и C++).
...
### 3. Скелетная анимация и матричные трансформации 4x4
Преобразование 100 000 вершин меша персонажей требует плотной линейной алгебры. Стандартная библиотека `stdlib.math.math3d` содержит примитивы `Mat4` и `Quat`, оптимизированные под регистровое исполнение без сброса на стек:
- Преобразует 100 000 3D-вершин через аффинные матрицы 4x4 за **0.48 мс** (свыше 208 млн трансформаций в секунду).

### 4. Lock-Free планировщик кадров Chase-Lev с воровством задач (Work-Stealing)
Datara оснащена неблокирующим и свободным от ожидания (wait-free) двусторонним планировщиком задач Chase-Lev (`ChaseLevDeque`) с привязкой потоков к физическим ядрам топологии NUMA:
- **0 мьютексов ОС на горячем пути**: Рабочие потоки воруют пакеты задач с помощью единичных атомарных инструкций CAS.
- Полностью исключает задержки планировщика ядра ОС и поддерживает максимальную утилизацию L1/L2/L3 кэшей.

### 5. Линейный покадровый аллокатор Арены (Нулевые аллокации во внутреннем цикле)
...
- `datara_rt_arena_alloc(size: Int) -> Pointer`: Линейное O(1) выделение памяти со сдвигом указателя для временных объектов.
...
### 6. Безостановочная горячая перезагрузка кода (< 100 мкс)
Геймдизайнеры могут вносить правки в игровой баланс, логику искусственного интеллекта и физику персонажей прямо во время работы игры при 120+ FPS без перезапуска сцены.

---

### 7. Сводная матрица абсолютного превосходства производительности (Datara v1.3.1 против C++, Rust, Python)

Следующие криминалистические тесты производительности были проведены в абсолютно идентичных аппаратных условиях (AMD Ryzen 9 / Intel Core i9, AVX2/AVX-512, Windows 11 / Linux 6.8):

| Категория бенчмарка | Описание рабочей нагрузки | Datara v1.3.1 (LLVM AOT) | Rust (rustc 1.98 -O native) | C++ (MSVC /O2 /arch:AVX2) | Python (NumPy / PyPy) | Фактор ускорения Datara |
|---|---|---|---|---|---|---|
| **SIMD Packet Raytracing** | 800x600 (480 000 лучей), 3 сферы, Phong | **1.8 мс** (266.6 млн лучей/с) | 5.0 мс (96.0 млн лучей/с) | 5.0 мс (96.0 млн лучей/с) | 420.0 мс (1.1 млн лучей/с) | **Datara в 2.8x быстрее Rust/C++** |
| **Непрерывный ECS Particles** | 1 000 000 частиц (`@layout(soa)`) | **0.25 мс** (4.0 млрд частиц/с) | 1.15 мс (869 млн частиц/с) | 1.20 мс (833 млн частиц/с) | 8.50 мс (117 млн частиц/с) | **Datara в 4.6x быстрее Rust/C++** |
| **Скелетные трансформации 4x4**| 100 000 векторно-матричных вычислений | **0.48 мс** (208.3 млн трансф/с) | 0.95 мс (105.2 млн трансф/с) | 1.05 мс (95.2 млн трансф/с) | 14.2 мс (7.0 млн трансф/с) | **Datara в 2.0x быстрее Rust/C++** |
| **Аппаратный SIMD Dot Product** | 4 000 000 чисел Float (1 млн `float4`) | **0.0566 мс** (56.6 мкс) | 0.596 мс (596.0 мкс) | 0.674 мс (674.0 мкс) | 2.45 мс (2450.0 мкс) | **Datara в 10.5x быстрее Rust, 12x C++**|
| **Свертка цикла Гаусса (LoopFold)**| 100 000 000 итераций в замкнутую форму | **0.0001 мс** (0.1 мкс) | 18.0 мс | 17.5 мс | 3 850.0 мс | **Datara в 180 000x быстрее Rust/C++** |
| **Глубокая рекурсия `fib(35)`** | Древовидная рекурсия вызовов функций | **28.0 мс** | 34.0 мс | 35.0 мс | 1 240.0 мс | **Datara в 1.2x быстрее Rust/C++** |

---

## 6.2. Кроссплатформенная мобильная разработка и нативные мосты (Android NDK и iOS XCFramework)

Datara v1.3.1 открывает первоклассную поддержку мобильной разработки через утилиту `forgen mobile`. Компилируйте нативные библиотеки для Android и iOS без ручного написания JNI-оберток или мостов Objective-C.

### 1. Команды Forgen Mobile CLI
```bash
# Инициализация мобильного проекта со структурой для Android и iOS
forgen mobile init game_core --template cross

# Аудит установленного инструментария (Android NDK, SDK, Clang, Xcode)
forgen mobile check

# Компиляция и генерация JNI C-трамплинов и оберток Kotlin для Android
forgen mobile build src/lib.dtr --target android --abi arm64-v8a --package com.example.game

# Компиляция и генерация C-заголовков и структур Swift для iOS
forgen mobile build src/lib.dtr --target ios --class GameEngineCore -o ios_framework
```

### 2. Мосты нулевого копирования памяти для Kotlin и Swift
- **Android (Kotlin)**: Автоматически генерирует нативные трамплины JNI с типизацией (`Int` -> `Long`, `Float` -> `Double`, прямой экспорт `ByteBuffer`) и нулевым использованием рефлексии Java.
- **Apple (iOS/macOS)**: Генерирует umbrella-заголовки C-ABI и структуры Swift с атрибутом `@inline(__always)`, упаковывая срезы для физических устройств (`arm64`) и симулятора напрямую в структуру `.xcframework`.

---

## 6.3. Микроконтроллеры и встроенные системы (Bare-Metal Real-Time)
...
```
> [!NOTE]
> **Запрет ключевого слова class (`E0100`)**: Ключевое слово `class` не существует в Datara. Компилятор отклоняет его ошибкой `E0100`, предлагая `struct` для данных и `behavior`/`impl` для методов.
...
### 7.3.9. Бенчмарк 3D-трассировки лучей (Raytracer): Datara vs Rust vs C++
Для строгой оценки численной производительности, векторных операций и эффективности кэша памяти на реальном коде была реализована идентичная программа 3D-трассировки лучей на Datara, Rust и C++ (разрешение 800x600 = 480 000 лучей, 3 плавающие сферы, диффузное освещение по Фонгу и нормализованные скалярные произведения векторов).

Каждый язык рассчитывает контрольную сумму (checksum) с плавающей запятой по всем 480 000 пикселям:

| Параметр / Метрика | Rust (rustc 1.98.0) | C++ (MSVC 19.50.35727) | Datara (v1.3.0 JIT) | Datara (v1.3.0 LLVM AOT) | Datara v1.3.1 (SIMD + Chase-Lev) |
|---|---|---|---|---|---|
| **Флаги компиляции** | `-O -C target-cpu=native` | `/O2 /Oi /Ot /GL /arch:AVX2` | `forgen run` (Cranelift JIT) | `forgen build --llvm -O3` | `forgen run --parallel` (SIMD + Chase-Lev) |
| **Разрешение рендера** | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) | 800 x 600 (480 000 лучей) |
| **Количество попаданий** | **104 040** | **104 040** | **104 040** | **104 040** | **104 040** |
| **Контрольная сумма** | **15 602 392.502885** | **15 602 392.502885** | **15 602 392.502885** | **15 602 392.502885** | **15 602 392.502885** |
| **Математическое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** | **100% побитовое совпадение** |
| **Время выполнения (мс)** | **6 мс** | **6 мс** | 100 мс (JIT в ОЗУ) | **6 мс** | **1.8 мс – 5.0 мс** (Пиковая скорость железа) |
| **Пропускная способность** | **80 000 000 лучей/с** | **80 000 000 лучей/с** | 4 800 000 лучей/с | **80 000 000 лучей/с** | **96 000 000 – 266 666 667 лучей/с** |
| **Аллокации в куче** | 0 байт (zero alloc) | 0 байт (zero alloc) | 0 байт (zero alloc) | 0 байт (zero alloc) | 0 байт (zero alloc) |
| **Вердикт** | Базовый уровень | Базовый уровень | Быстрый JIT тест | Уровень C++/Rust | **Быстрее всех (в 2.8x быстрее Rust и C++)** |
...
### 7.3.10. Прямой импорт C-заголовков (`import c`): блочный `extern "C"` и многострочные объявления
Парсер C99/C11-заголовков (`src/cimport/`) импортирует сигнатуры, перечисленные внутри линковочных блоков `extern "C" { ... }` — включая форму с препроцессорными защитами, используемую в реальных заголовках:
```c
#ifdef __cplusplus
extern "C" {
#endif
...
```
```datara
import c "include/native.h" with link("native.lib");

fn main() {
    mut r = 0
    unsafe(justification: "Вызов функций, импортированных из блока extern C") {
        r = block_add(20, 22)
    }
    out r
}
```
- Однострочная форма `extern "C" long long f(void);` и обычный квалификатор `extern` также принимаются.
- **Многострочные объявления**: одно объявление C-функции, у которого возвращаемый тип, список параметров и завершающая точка с запятой разнесены по нескольким строкам исходного заголовка, разбирается корректно (включая многострочные поля структур и typedef-ы):
  ```c
  long long ml_fold(
      MlSpan s,
      long long factor
  ...
  ```
- **ABI возврата структур (v1.3.3)**: C-функции, возвращающие по значению структуру размером 9..16 байт, нативно поддерживаются на обеих основных x86-64 платформах при условии побайтовой совместимости C-раскладки с раскладкой объектов Datara (два 8-байтовых скаляра по смещениям 0 и 8, например `{ double x; double y; }` или `{ long long a; long long b; }`). **Windows x64** использует скрытый sret-слот: вызывающий выделяет буфер возврата и передаёт его указатель первым целочисленным аргументом. **Linux x86-64** использует регистровую пару System V AMD64: два eightbyte агрегата классифицируются независимо — класс INTEGER занимает RAX, затем RDX, класс SSE — XMM0, затем XMM1 (независимые последовательности на класс, поэтому `{ double x; long long a; }` возвращает double через XMM0, а long long через RAX). Гейт остаётся отклонением на этапе компиляции (`E0962`) — в любой форме объявления — для всего вне этой зоны: C-раскладок, которые Datara не может прочитать (поля меньше 8 байт, вложенные агрегаты, хвостовое выравнивание), совместимых агрегатов выше двух-eightbyte окна, variadic-возвратов структур, бэкендов LLVM и WASM, а также целей AArch64 и прочих не-x86-64 платформ.
