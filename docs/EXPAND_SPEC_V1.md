...
| 3 | Чип как зависимость: `[hardware] chip = "stm32f407@rev.C"` | несовпадение `hw_rev` → `E-EMB-010` |
...
### EMB-9 — Драйверы как sparks-пакеты — E8 (≈1 неделя)

| # | Шаг | Гейт |
| --- | --- | --- |
| 1 | `dpm add spark:drv-bmi270` — драйвер приходит с манифестом `hardware` | `dpm install` + `forgen build` |
| 2 | Ресурсы драйвера (I2C-адрес, IRQ, регионы) в capabilities | `forgen verify --hardware` |
| 3 | Версионирование по `hw_rev`: драйвер объявляет, на каких ревизиях работает | несовпадение → `E-EMB-010` |
| 4 | Диагностика: «не хватает `i2c1` — он занят пакетом `drv-xxx`» | тест на конфликт пакетов |

**E8 (нововведение):** пакетный менеджер, который знает про железо. `cargo` не знает, что у тебя один
UART; `dpm` — знает и не даст поставить два драйвера на один периферийный блок.
...
### 37.8 E8 — драйверы как sparks-пакеты с hardware-ресурсами

```toml
# манифест пакета drv-bmi270
[package]
name = "drv-bmi270"
[hardware]
requires = ["i2c1@0x68", "irq:EXTI4"]
supports_hw_rev = ["A", "B", "C"]
```
**Проверяет:** `dpm` не даст поставить два пакета на один ресурс; `E-EMB-010` при несовпадении `hw_rev`.
**Чем лучше:** `cargo` не знает про железо вообще — тут пакетный менеджер становится частью системы типов
железа. **Гейт:** тест на конфликт ресурсов между двумя пакетами.
...
| Драйверы | crates без модели железа | копипаста | sparks-пакеты с ресурсами |
...
| `E-EMB-007` | Обращение к hardware-ресурсу без capability | добавь ресурс в `[hardware] requires` пакета |
...
| `E-EMB-010` | `hw_rev` чипа не поддерживается драйвером | обнови драйвер или укажи `supports_hw_rev` |
...
| 9 | Драйвер без `[hardware] requires` обращается к UART | `E-EMB-007` |
...
| 12 | Драйвер как sparks-пакет (E8) | `dpm add spark:drv-bmi270` + `forgen build` |
...
| Полный набор драйверов в sparks | экосистема, растёт сама через E8 | после релиза |
...
| EMB-9 | EMB-8, EMB-2 | релиз WP-10 | 1 нед | `feat(emb): E8 drivers as sparks` |
...
| `project/manifest.rs` | `[target]` + `[hardware]` рядом с `[capabilities]` | EMB-0/EMB-2 синхронно с WP-6 (dpm) |
...
v1.5    — eBPF-бэкенд, WCET-отчёт, драйверы в sparks
