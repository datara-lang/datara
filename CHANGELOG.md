...
- **Maintenance pass (clippy/lint hygiene)**: whole-program dead-code lint (cross-module reference scanning; `Match`/`Decide`/closures/contracts/globals fully covered — zero false positives on multi-file projects), `W-SYN-002` canonical type-spelling warnings actually emitted, cargo clippy warnings 92 → 0, legacy `try`/`catch` AST/DMIR plumbing removed (55 sites), integer-literal `return` unification with the function return type fixed, `forgen check`/`forgen lint` operate on the merged module set.
- **CI hardening**: Unix bridge JIT loader fixed (`bridge_mmap` → libc `mmap` — was an E0425 build failure on Linux/macOS/MSRV/ASan), Windows SIMD LLVM-IR test updated to the v1.4.5 emitter style, examples-compile-budget test made cold-start resilient (warm pass measured), Sparks remote test seeds generated.
- **Documentation portal on GitHub Pages**: `docs/` (30 Markdown documents: Tutorial, Language Reference Manual, Spec V1/V2, Glossary, Conformance Matrix, Performance, RU guide, interop guides) deploys to https://datara-lang.github.io/datara/ via a Jekyll Pages workflow; interactive dashboard `docs/index.html` links into the generated reference pages.
...
### Changed
- **Prelude deduplication**: duplicate 4-Tier memory signature block removed (resolves the conflicting `arena_reset` Int-vs-Unit registration in favor of the runtime-accurate `(Int) -> Unit`); string builtin registrations consolidated into paired canonical/`datara_rt_` loops. The documented `and`/`xor`/`or` bitwise aliases remain registered (README parity; restored after an accidental v1.4.5 dropout).
...
- **LLVM SIMD `dot()` returning 0 (v1.4.5 final)**: native SIMD local variables now lower to typed `<N x T>` allocas (16-byte aligned) instead of opaque scalar slots, which keeps the vector type alive through loads/stores on the LLVM backend — `dot(a, b)` for `float4` operands compiles to `fmul` + `llvm.vector.reduce.fadd` and returns the correct value (the Cranelift path was never affected). The accompanying LLVM-IR test asserts the typed-vector emitter style.
