...
- **CI hardening**: Unix bridge JIT loader fixed (`bridge_mmap` → libc `mmap` — was an E0425 build failure on Linux/macOS/MSRV/ASan), Windows SIMD LLVM-IR test updated to the v1.4.5 opaque-pointer emitter style, examples-compile-budget test made cold-start resilient (warm pass measured), Sparks remote test seeds generated.
