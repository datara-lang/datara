...
impl<'a> Parser<'a> {
    ...
    pub(crate) fn parse_primary(&mut self) -> Option<Expr> {
        ...
        match token.token_type {
            ...
            TokenType::From => Some(Expr::Identifier("from".into(), token.span)),
            TokenType::Shared => Some(Expr::Identifier("shared".into(), token.span)),
            TokenType::Where => Some(Expr::Identifier("where".into(), token.span)),
            TokenType::Class => Some(Expr::Identifier("class".into(), token.span)),
            TokenType::Own => Some(Expr::Identifier("own".into(), token.span)),
            TokenType::Type => Some(Expr::Identifier("type".into(), token.span)),
            TokenType::Require => Some(Expr::Identifier("require".into(), token.span)),
