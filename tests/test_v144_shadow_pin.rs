...
fn test_shadow_heap_pinning_lifecycle() {
    ...
    unsafe {
        ...
        let var_name = c"features".as_ptr();
        ...
        assert_eq!(
            datara_py_shadow_pin_count(),
            1,
            "must not duplicate pin slot"
        );
        assert!(
            elapsed.as_nanos() < 50_000,
            "repeat pin must be sub-microsecond cache hit: {}ns",
            elapsed.as_nanos()
        );
...
fn test_shadow_heap_pinning_multiple_buffers() {
    ...
    unsafe {
        ...
        let name_a = c"buf_a".as_ptr();
        let name_b = c"buf_b".as_ptr();
